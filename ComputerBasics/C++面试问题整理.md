...
**普通递归函数**：在每次调用递归函数时回调用当前函数的栈帧(存储变量和函数返回地址)，当递归深度太深时会导致 `stack overflow`。**每次调用递归函数都需要创建新的栈帧，依次压栈**。

**尾递归函数**：满足两个条件，递归调用是函数的最后一步操作，不需要参与其它计算。复用当前栈帧，直接传递结果。尾递归的时间复杂度与空间复杂度适合循环实现是一样的，与循环等价。

```cpp
// 普通递归
int sum(int n) {
    if (n == 0) return 0;
    return n + sum(n - 1); // 递归调用后还有加法操作，不是尾递归
}

// 尾递归
int sum_tail(int n, int total = 0) {
    if (n == 0) return total;
    return sum_tail(n - 1, total + n); // 递归调用是函数的最后一步操作
}
```
