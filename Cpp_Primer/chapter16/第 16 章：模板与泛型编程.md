- [16.1 定义模板](#161-定义模板)
    - [函数模板（Function Templates）](#函数模板function-templates)
    - [类模板（Class Templates）](#类模板class-templates)
    - [模板参数（Template Parameters）](#模板参数template-parameters)
    - [成员模板（Member Templates）](#成员模板member-templates)
    - [控制实例化（Controlling Instantiations）](#控制实例化controlling-instantiations)
    - [效率与灵活性（Efficiency and Flexibility）](#效率与灵活性efficiency-and-flexibility)
  - [模板实参推断（Template Argument Deduction）](#模板实参推断template-argument-deduction)
    - [类型转换与模板类型参数（Conversions and Template Type Parameters）](#类型转换与模板类型参数conversions-and-template-type-parameters)
    - [函数模板显式实参（Function-Template Explicit Arguments）](#函数模板显式实参function-template-explicit-arguments)
    - [尾置返回类型与类型转换（Trailing Return Types and Type Transformation）](#尾置返回类型与类型转换trailing-return-types-and-type-transformation)
    - [函数指针和实参推断（Function Pointers and Argument Deduction）](#函数指针和实参推断function-pointers-and-argument-deduction)
    - [模板实参推断和引用（Template Argument Deduction and References）](#模板实参推断和引用template-argument-deduction-and-references)
    - [理解std::move（Understanding std::move）](#理解stdmoveunderstanding-stdmove)
    - [转发（Forwarding）](#转发forwarding)
  - [重载与模板（Overloading and Templates）](#重载与模板overloading-and-templates)
  - [可变参数模板（Variadic Templates）](#可变参数模板variadic-templates)
    - [编写可变参数函数模板（Writing a Variadic Function Template）](#编写可变参数函数模板writing-a-variadic-function-template)
    - [包扩展（Pack Expansion）](#包扩展pack-expansion)
    - [转发参数包（Forwarding Parameter Packs）](#转发参数包forwarding-parameter-packs)
  - [模板特例化（Template Specializations）](#模板特例化template-specializations)
