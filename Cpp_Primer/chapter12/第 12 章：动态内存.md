- [第12章 动态内存](#第12章-动态内存)
  - [动态内存与智能指针（Dynamic Memory and Smart Pointers）](#动态内存与智能指针dynamic-memory-and-smart-pointers)
    - [shared\_ptr类（The shared\_ptr Class）](#shared_ptr类the-shared_ptr-class)
    - [直接管理内存（Managing Memory Directly）](#直接管理内存managing-memory-directly)
    - [shared\_ptr和new结合使用（Using shared\_ptrs with new）](#shared_ptr和new结合使用using-shared_ptrs-with-new)
    - [智能指针和异常（Smart Pointers and Exceptions）](#智能指针和异常smart-pointers-and-exceptions)
    - [unique\_ptr（unique\_ptr）](#unique_ptrunique_ptr)
    - [weak\_ptr（weak\_ptr）](#weak_ptrweak_ptr)
  - [动态数组（Dynamic Arrays）](#动态数组dynamic-arrays)
    - [new和数组（new and Arrays）](#new和数组new-and-arrays)
    - [allocator类（The allocator Class）](#allocator类the-allocator-class)
