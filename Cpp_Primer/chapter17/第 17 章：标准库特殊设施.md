- [第17章 标准库特殊设施](#第17章-标准库特殊设施)
  - [tuple类型（The tuple Type）](#tuple类型the-tuple-type)
    - [定义和初始化tuple（Defining and Initializing tuples）](#定义和初始化tupledefining-and-initializing-tuples)
    - [使用tuple返回多个值（Using a tuple to Return Multiple Values）](#使用tuple返回多个值using-a-tuple-to-return-multiple-values)
  - [bitset类型（The bitset Type）](#bitset类型the-bitset-type)
    - [定义和初始化bitset（Defining and Initializing bitsets）](#定义和初始化bitsetdefining-and-initializing-bitsets)
    - [bitset操作（Operations on bitsets）](#bitset操作operations-on-bitsets)
  - [正则表达式（Regular Expressions）](#正则表达式regular-expressions)
    - [使用正则表达式库（Using the Regular Expression Library）](#使用正则表达式库using-the-regular-expression-library)
    - [匹配与Regex迭代器类型（The Match and Regex Iterator Types）](#匹配与regex迭代器类型the-match-and-regex-iterator-types)
    - [使用子表达式（Using Subexpressions）](#使用子表达式using-subexpressions)
    - [使用regex\_replace（Using regex\_replace）](#使用regex_replaceusing-regex_replace)
  - [随机数（Random Numbers）](#随机数random-numbers)
    - [随机数引擎和分布（Random-Number Engines and Distribution）](#随机数引擎和分布random-number-engines-and-distribution)
    - [其他随机数分布（Other Kinds of Distributions）](#其他随机数分布other-kinds-of-distributions)
  - [IO库再探（The IO Library Revisited）](#io库再探the-io-library-revisited)
    - [格式化输入与输出（Formatted Input and Output）](#格式化输入与输出formatted-input-and-output)
    - [未格式化的输入/输出操作（Unformatted Input/Output Operations）](#未格式化的输入输出操作unformatted-inputoutput-operations)
    - [流随机访问（Random Access to a Stream）](#流随机访问random-access-to-a-stream)
