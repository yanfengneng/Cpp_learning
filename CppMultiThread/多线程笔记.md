...
[第一版：C++并发编程实战](https://github.com/xiaoweiChen/Cpp_Concurrency_In_Action?tab=readme-ov-file)、[第二版：C++并发编程实战](https://github.com/xiaoweiChen/CPP-Concurrency-In-Action-2ed-2019)
...
如果需要在同一个线程中运行多个不同的函数，**可以将它们组合在一起，放入一个函数中**，或者使用 lambda 表达式、**函数对象(也就是了lambda 表达式的类实现)**等进行封装。
...
- `std::condition_variable` 必须与 `std::mutex` 一起使用，**以确保在检查和修改条件时的原子性和线程安全性。**
...
* 一般与 `std::mutex` 和 `std::unique_lock<std::mutex>` 配合使用，**线程在条件变量上等待，直到另一个线程发出通知。**
...
**线程池维护着多个线程，这避免了在处理短时间任务是，创建与销毁线程的代价。**
